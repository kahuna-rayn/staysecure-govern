...
export const handleCreateUser = async (
  ...
  try {
    ...
    // Only send auth-related fields to the Edge Function
    const userData = {
      email: newUser.email,
      password: newUser.password,
      full_name: newUser.full_name // This should set display_name in auth.users
    };
...
export const handleDeleteUser = async (userId: string) => {
  try {
    ...
    // === EDGE FUNCTION CALL WITH AUTHENTICATION ===
    console.log('🚀 Attempting Edge Function call with authentication...');
    console.log('🚀 User ID to delete:', userId);
    console.log('🚀 Edge Function URL: https://ufvingocbzegpgjknzhm.supabase.co/functions/v1/delete-user');
    
    // Use supabase.functions.invoke() instead of fetch to handle CORS automatically
    const { data: deleteResult, error: deleteError } = await supabase.functions.invoke('delete-user', {
      body: { userId }
    });

    console.log('🚀 Edge Function response:', { data: deleteResult, error: deleteError });
    ...
    console.log('🚀 Edge Function success result:', deleteResult);
