...
const PhysicalLocationTab: React.FC<PhysicalLocationTabProps> = ({ profile }) => {
  ...
  const { data: locationAccess = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      console.log('Fetching physical location access for email:', profile.email);
      
      const { data, error } = await supabase
        .from('physical_location_access')
        .select('access_purpose, status, date_access_created, date_access_revoked')
        .eq('user_id', profile.id)
        .order('created_at', { ascending: false });
      
      console.log('Physical location access query result:', { data, error });
  ...
  const handleUpdate = async (id: string, updates: Partial<PhysicalLocationAccess>) => {
    try {
      console.log('Updating physical location access:', id, updates);
      ...
      console.log('Physical location access update result:', { error });
