...
const EnhancedMetrics: React.FC = () => {
  ...
  const mainMetrics: MetricDefinition[] = [
    ...
    },
    {
      ...
      getValue: (profiles) => {
        const scores = profiles.filter(p => p.cyber_learner).map(() => Math.floor(Math.random() * 40) + 60);
    ...
    },
    {
      ...
      icon: <TrendingUp className="h-6 w-6" />,
      getValue: (profiles) => {
        const scores = profiles.filter(p => p.dpe_learner).map(() => Math.floor(Math.random() * 40) + 60);
        return scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
      },
      drillDownLevels: ['Average Score', 'Location', 'Department', 'Staff Scores'],
      type: 'score'
    ...
    },
    {
      ...
      icon: <Globe className="h-6 w-6" />,
      getValue: (profiles) => profiles.filter(p => p.language === 'Chinese').length,
      drillDownLevels: ['Organization', 'Location', 'Department'],
      type: 'count'
    }
  ];
